                                ...
                                <tbody>
                                    @foreach ($owner as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ asset('image/reg-owner/' . $item->image_idcard_back) }}">
                                                    <img style="width: 150px"
                                            ...
                                            <td class="btn bg-warning">
                                                {{ $item->status == 2 ? 'Pending' : ($item->status == 1 ? 'Approved' : 'Rejected') }}
                                            </td>
                                            <td>
                                                <button class="bg-success"><a
                                                        href="{{ route('admin.owner.accept', ['id' => $item->user_id]) }}">Accept</a></button>
                                                <button class="bg-danger"><a
                                                        href="{{ route('admin.owner.reject', ['id' => $item->user_id]) }}">Reject</a></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
